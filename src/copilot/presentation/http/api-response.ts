...
export type CopilotApiSuccessResponse<T> = {
  data: T;
  meta: {
    generatedAt: string;
  };
...
export type CopilotApiErrorResponse = {
  error: {
    code: string;
    message: string;
  };
...
export function jsonSuccess<T>(data: T, status = 200) {
  return NextResponse.json<CopilotApiSuccessResponse<T>>(
    {
      data,
      meta: {
...
export function jsonError(error: ApplicationError, status = mapErrorStatus(error)) {
  return NextResponse.json<CopilotApiErrorResponse>(
    {
      error: {
        code: error.code,
