...
export type PredictiveApiSuccessResponse<T> = {
  data: T;
  meta: {
    generatedAt: string;
  };
...
export type PredictiveApiErrorResponse = {
  error: {
    code: string;
    message: string;
  };
...
export function jsonSuccess<T>(data: T, status = 200) {
  return NextResponse.json<PredictiveApiSuccessResponse<T>>(
    {
      data,
      meta: {
...
export function jsonError(error: ApplicationError, status = mapErrorStatus(error)) {
  return NextResponse.json<PredictiveApiErrorResponse>(
    {
      error: {
        code: error.code,
