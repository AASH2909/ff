...
export type AISummaryApiSuccessResponse<T> = {
  data: T;
  meta: {
    generatedAt: string;
  };
...
export type AISummaryApiErrorResponse = {
  error: {
    code: string;
    message: string;
  };
...
export function jsonSuccess<T>(data: T, status = 200) {
  return NextResponse.json<AISummaryApiSuccessResponse<T>>(
    {
      data,
      meta: {
...
export function jsonError(error: ApplicationError, status = mapErrorStatus(error)) {
  return NextResponse.json<AISummaryApiErrorResponse>(
    {
      error: {
        code: error.code,
