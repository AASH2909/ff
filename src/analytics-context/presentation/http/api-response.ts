...
export type AnalyticsContextApiSuccessResponse<T> = {
  data: T;
  meta: {
    generatedAt: string;
  };
...
export type AnalyticsContextApiErrorResponse = {
  error: {
    code: string;
    message: string;
  };
...
export function jsonSuccess<T>(data: T, status = 200) {
  return NextResponse.json<AnalyticsContextApiSuccessResponse<T>>(
    {
      data,
      meta: {
...
export function jsonError(error: ApplicationError, status = mapErrorStatus(error)) {
  return NextResponse.json<AnalyticsContextApiErrorResponse>(
    {
      error: {
        code: error.code,
