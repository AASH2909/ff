...
export type TimelineApiSuccessResponse<T> = {
  data: T;
  meta: {
    generatedAt: string;
  };
...
export type TimelineApiErrorResponse = {
  error: {
    code: string;
    message: string;
  };
...
export function jsonSuccess<T>(data: T, status = 200) {
  return NextResponse.json<TimelineApiSuccessResponse<T>>(
    {
      data,
      meta: {
...
export function jsonError(error: ApplicationError, status = mapErrorStatus(error)) {
  return NextResponse.json<TimelineApiErrorResponse>(
    {
      error: {
        code: error.code,
