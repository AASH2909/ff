...
export type DecisionApiSuccessResponse<T> = {
  data: T;
  meta: {
    generatedAt: string;
  };
...
export type DecisionApiErrorResponse = {
  error: {
    code: string;
    message: string;
  };
...
export function jsonSuccess<T>(data: T, status = 200) {
  return NextResponse.json<DecisionApiSuccessResponse<T>>(
    {
      data,
      meta: {
...
export function jsonError(error: ApplicationError, status = mapErrorStatus(error)) {
  return NextResponse.json<DecisionApiErrorResponse>(
    {
      error: {
        code: error.code,
