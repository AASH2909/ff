...
export default function InventoryPage() {
  ...
  const { completeInventoryAudit } = useOperationalActions();
  ...
  return (
    <>
      <PageHeading
        ...
        actions={canAudit ? <Button size="sm" variant="secondary" asChild onClick={completeInventoryAudit}><Link href={canOpenPos ? "/pos" : defaultRoute}>{canOpenPos ? t("pages.inventory.openPos") : t("pages.inventory.completeAudit")}</Link></Button> : undefined}
      />
      <PageSection className="space-y-3 px-4 pb-4 sm:px-6 lg:px-8">
        ...
        <div className="rounded-lg border bg-background/70 p-4 text-sm text-muted-foreground">
          <p className="font-semibold text-foreground">{t("pages.inventory.openedFrom")} {t(state.openedFromKey)}</p>
      ...
      <PageSection className="grid gap-3 sm:grid-cols-3">
        ...
        <MetricTile label={t("pages.inventory.lowStock")} value={String(state.inventoryAlertCount)} helper={t("pages.inventory.needsReorder")} trend={<TriangleAlert className="size-5 text-warning" />} />
